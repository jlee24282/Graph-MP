...
public class ConnectedComponents {
	...
	private Integer[] id = null; // id[v] = id of connected component containing
									// v
	...
	public int computeCCWholeGraph(ArrayList<ArrayList<Integer>> wholeGraphList) {
		...
		/**
		 * these private variables are used for computing the connected
		 * components of subgraph induced by Integer[] A
		 */
	...
	public boolean checkConnectivity(Integer[] A) {
		...
		Boolean[] marked = new Boolean[A.length]; // marked[v] = has vertex v
													// been
		// marked?
		Integer[] id = new Integer[A.length]; // id[v] = id of connected
												// component
		// containing v
		Integer[] size = new Integer[A.length]; // size[id] = number of vertices
												// in
		// given component
	...
	public boolean checkConnectivity() {
		if (this.numConnectedComponents != 1) {
			System.out.println("this graph is not connected, number of connected components in graph is : "
			...
			System.out.println("after update the graph structure, the number of connected components in graph is : "
					+ this.computeCCSubGraph(V));
			return false;
		} else {
			return true;
	...
	public int computeCCSubGraph(Integer[] A) {
		...
		/** map to original nodes ID */
		ArrayList<ArrayList<Integer>> comps = new ArrayList<ArrayList<Integer>>(count);
	...
	/**
	 * @param subGraphList
	 *            reduced sub-graph adjacency list
	 * @param v
	 *            starting node
	 */
	...
	//	APDMInputFormat apdm = new APDMInputFormat("data/SimulationData/Protest/APDM-GridData-100-precen_0.1-noise_0-numCC_1_0.txt");
		...
		System.out.println("[0 1 4 6] should be [3]: " + c.computeCCSubGraph(new Integer[] { 0, 1, 4, 6 }));
		System.out.println("[0 1 2 3 4 6] should be [2]: " + c.computeCCSubGraph(new Integer[] { 0, 1, 2, 3, 4, 6 }));
		System.out.println(
				"[0 3 4 5 88 91 ] should be [4]: " + c.computeCCSubGraph(new Integer[] { 0, 3, 4, 5, 88, 91 }));
