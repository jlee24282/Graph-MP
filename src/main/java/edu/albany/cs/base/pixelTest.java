...
public class pixelTest {
    ...
    public void genData(ArrayList<Edge> edges, ArrayList<Edge> treEdges, String outPutFileName){
        double[] weight = new double[graphSize];
        ...
        for (int j = 0; j < graphSize; j++) {
            if (nodes.contains(j)) {
                weight[j] = (int) normAbnormalNodes.sample();
            } else {
                weight[j] = (int) normNormalNodes.sample();
            }
            if(weight[j] < 0)
                weight[j] = 0;
            else if(weight[j] >255)
                weight[j] = 255;
        }


        APDMInputFormat.generateAPDMFile_bot("Test", "RandomData", edges, weight,
    ...
    public static void main(String args[]) throws IOException {
        ...
        new pixelTest(args).generateSingleCase();
